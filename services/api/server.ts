                    ...
                    (
                      SELECT json_agg(
                               json_build_object(
                                 ...
                                 'region_territory', cfc.region_territory,
                                 'region_language', cfc.region_language,
  ...
  app.get("/api/master/ledgers", async (_req, res) => {
    try {
      ...
      if (ids.length > 0) {
        ...
        // territory/language(許諾地域・言語の枠。0117)と、当該マテリアルの根拠文書番号
        //   (condition_lines.source_material_id 経由の最新 license 器の文書番号)も返す。
        //   これが無いと個別利用許諾フォームで既存マテリアルを選んでも地域/言語/根拠文書が
        //   既定値(「1-1に準ずる」「本条件書(新規)」)に化け、登録内容と食い違う。
        let mats: any;
        try {
          mats = await query(
