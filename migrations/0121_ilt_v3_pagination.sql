-- 0121_ilt_v3_pagination.sql
-- individual_license_terms_v3 テンプレの改ページ・余白を自然化して DB(current版)へ反映する。
--   経緯: 0087 で登録、0118 で案X更新。sync-templates-to-db.mjs は v3 を同期しない(config非キー)ため、
--         v3 のテンプレ変更は本マイグレーション(0087/0118 と同方式・冪等)で新版を積む必要がある。
--   内容: services/worker/templates/individual_license_terms_v3.html と同一(案X + 改ページ制御)。
--   変更点: 見出し/リード文 break-after:avoid(孤立防止) / 行 break-inside:avoid /
--           成長・高さのある表(1-1/1-2/1-3A/1-3B/1-4/2-1/2-3A/3運用/4特約)の avoid-break を外し
--           該当ヘッダを thead 化(各ページでヘッダ繰り返し・表ジャンプの余白解消) / 署名 break-inside:avoid。
DO $mig$
DECLARE
  tid INTEGER;
  vid INTEGER;
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  /* 自然な改ページ: 見出しは次の内容と一緒に送る(孤立防止)、行はセル内で割らない、
     表ヘッダは改ページ時に各ページで繰り返す、署名ブロックは分割しない。 */
  h2.block,h3.sec{break-after:avoid;page-break-after:avoid;break-inside:avoid;page-break-inside:avoid;}
  p.lead,.sub-title{break-after:avoid;page-break-after:avoid;}
...
<table style="font-size:8.4pt;">
  <thead><tr><th style="width:84pt;">用語</th><th>定義</th><th class="c" style="width:56pt;">参照</th></tr></thead>
...
<table>
  <thead><tr><th>相手先</th><th>地域</th><th>言語</th><th>適用条件</th><th>個別料率</th><th>締結日</th><th>備考</th></tr></thead>
  {{#each sublicensees}}
...
<table style="font-size:8.4pt;">
  <thead><tr><th style="width:80pt;">版</th><th style="width:110pt;">計算基準日となる事由</th><th>備考</th></tr></thead>
  {{#each calcBaseRows}}
...
<table>
  <thead><tr><th style="width:82pt;">区分</th><th style="width:84pt;">時点</th><th>内容</th></tr></thead>
...
$ilt_v3$,
            '[]'::jsonb,
            'v3 改ページ・余白の自然化: 見出し孤立/表ジャンプ/署名分割の解消 (0121)', 'migration-0121')
