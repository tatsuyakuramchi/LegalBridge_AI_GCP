-- 0057_inspection_zero_fee.sql
-- 検収書: 支払対価が0の明細(委託報酬が利用許諾料に含まれる行)は「利用許諾料に含む」と表記し、¥0を避ける。
      ...
      {{#if (gt (length delivery_line_items) 0)}}
        {{#each delivery_line_items}}
          <tr>
            ...
            <td class="right">{{#if (gt (or inspected_amount_ex_tax amount_ex_tax) 0)}}{{formatYen (or inspected_amount_ex_tax amount_ex_tax)}}{{else}}<span style="font-size:8pt;color:#92400e;">利用許諾料に含む</span>{{/if}}</td>
          </tr>
        {{/each}}
      {{else}}
        <tr>
