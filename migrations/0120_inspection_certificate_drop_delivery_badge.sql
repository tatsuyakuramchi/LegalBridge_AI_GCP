-- 0120_inspection_certificate_drop_delivery_badge.sql
-- 検収書(inspection_certificate)テンプレから分納バッジ「分割納品 第N回 / 全M回予定」を削除。
--   複数業務明細×分納で「第N回/全M回」の回数集計(deliveryNo/totalDeliveries)が不整合に
--   なる(例: 第2回/全1回)ため廃止。分納の内訳は「今回の納品内容」の 今回数量/歩留率 列で表現。
--   0119 は適用済み(不変)のため、本差し替えを新版として current に。
--   disk: services/worker/templates/inspection_certificate.html と同一内容。0088/0119 と同一 upsert 方式。
DO $mig$
DECLARE
  tid INTEGER;
  cur_html TEXT;
...
BEGIN
  ...
  IF tid IS NULL THEN
    RAISE NOTICE '0120: inspection_certificate template not found, skipping';
    RETURN;
  END IF;
...
$ins_html$ THEN
    RAISE NOTICE '0120: inspection_certificate html already current, skipping';
    RETURN;
  END IF;
...
$ins_html$,
          ...
          '検収書テンプレ: 分納バッジ削除(0120)', 'migration-0120')
