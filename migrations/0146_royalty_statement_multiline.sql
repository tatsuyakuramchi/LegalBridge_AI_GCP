  ...
  <table style="margin-bottom:12px;">
    <thead>
      <tr>
        ...
        <th style="width:20%;">売上（{{intakeCurrency}}）</th>
        {{#unless (eq intakeCurrency "JPY")}}<th style="width:20%;">売上（円）</th>{{/unless}}
        <th style="width:10%;">料率</th>
        <th style="width:20%;">支払額（税抜）</th>
      </tr>
    </thead>
    <tbody>
      {{#each lines}}
      <tr>
        ...
        <td class="right">{{#unless (eq ../intakeCurrency "JPY")}}{{../intakeCurrency}} {{/unless}}{{this.salesInputStr}}</td>
        {{#unless (eq ../intakeCurrency "JPY")}}<td class="right">¥{{this.salesJpyStr}}</td>{{/unless}}
        <td class="center">{{this.ratePctResolved}}%</td>
        <td class="right">¥{{this.paymentJpyStr}}</td>
      </tr>
      {{/each}}
      <tr class="total-row">
        <td class="right" colspan="{{#if (eq intakeCurrency "JPY")}}3{{else}}4{{/if}}">支払合計（税抜）　※売上合計（円） ¥{{linesTotalSalesStr}}</td>
        <td class="right">¥{{linesTotalPaymentStr}}</td>
      </tr>
      <tr>
        <td class="right" colspan="{{#if (eq intakeCurrency "JPY")}}3{{else}}4{{/if}}">消費税（{{taxRate}}%）</td>
        <td class="right">¥{{linesTaxStr}}</td>
      </tr>
      <tr class="total-row">
        <td class="right" colspan="{{#if (eq intakeCurrency "JPY")}}3{{else}}4{{/if}}">源泉徴収税計算前　お支払予定額合計（税込）</td>
