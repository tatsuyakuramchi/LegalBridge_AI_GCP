-- 0044_notice_clause_templates.sql
-- 基本契約3種(業務委託/ライセンス/出版・個人/出版・法人)の DB テンプレ(document_templates)を
-- 頭書きの通知先(相手方の担当者/電話/メール)行 + 通知条項(第23条/第32条)を含む版へ更新する。
-- TEMPLATE_SOURCE=db の Search/worker が現行版を読むため、新 version を作成し current_version_id を付替える。
-- disk テンプレ(services/worker/templates/*.html)と templates_config.json の field_schema に整合。
-- 冪等: migration は schema_migrations で1回のみ適用。current が既に同一HTMLでも新版を積むだけで無害。
...
<div class="contract-document">
    ...
    <table class="tobogaki">
        <tbody>
            ...
            <tr>
                <td class="col-item">
                    通知先（乙）<span class="art-ref">（第23条）</span>
...
-- ===== license_master =====
WITH t AS (
  SELECT id FROM document_templates WHERE template_key = 'license_master'
), nv AS (
  INSERT INTO document_template_versions
      (template_id, version_no, html_source, field_schema, comment, created_by)
  SELECT t.id,
...
<div class="contract-document">
  ...
  <table class="tobogaki">
    <tbody>
      ...
      <!-- 基本条件 -->
      <tr>
        <td class="col-item">通知先（相手方）<span class="art-ref">（第23条）</span><br><span class="sub-note">本契約上の通知の宛先</span></td>
        <td class="col-value">
          担当者：{{NOTICE_CONTACT_NAME}}<br>
          電話：{{NOTICE_CONTACT_PHONE}}<br>
          E-mail：{{NOTICE_CONTACT_EMAIL}}
...
WITH t AS (
  SELECT id FROM document_templates WHERE template_key = 'pub_master_individual'
), nv AS (
  INSERT INTO document_template_versions
      (template_id, version_no, html_source, field_schema, comment, created_by)
  SELECT t.id,
...
<div class="contract-document">
  ...
  <table class="tobogaki">
    <tbody>
      ...
      <tr>
        <td class="col-item">通知先（許諾者）<span class="art-ref">（第３２条）</span><br><span class="sub-note">本契約上の通知の宛先</span></td>
        <td class="col-value">
          担当者：{{通知先担当者}}<br>
          電話：{{通知先電話}}<br>
          メール：{{通知先メール}}
...
WITH t AS (
  SELECT id FROM document_templates WHERE template_key = 'pub_master_corporate'
), nv AS (
  INSERT INTO document_template_versions
      (template_id, version_no, html_source, field_schema, comment, created_by)
  SELECT t.id,
