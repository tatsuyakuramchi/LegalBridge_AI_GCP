-- 0152_inspection_certificate_grouped.sql
-- 検収書テンプレートを「支払日グループ化フォーマット」へ刷新し、明細別の変更履歴を追加する。
--   本番は TEMPLATE_SOURCE=db (document_templates 読取) のため、disk の
--   services/worker/templates/inspection_certificate.html 変更だけでは反映されない。
--   本マイグレーションで DB の inspection_certificate を新バージョンへ更新する
--   (0044/0050 方式: 新 version を INSERT し current_version_id を差し替え)。
--
--   反映内容 (PR #478 / commits 31da753, 8b5657d, aa52ece):
--     - 明細を「支払日」でグループ化。支払済/支払予定バッジ、グループごとに
--       税抜小計/消費税/源泉徴収税額計算前 支払額(税込)。歩留率/数量列は撤去。
--       書類全体の税サマリー/総支払額は非表示。従来の特記(支払条件/振込先/みなし同意)は維持。
--     - paid_date が1件も無い明細は従来フラット表示に自動フォールバック(後方互換)。
--     - 明細別の発注条件変更履歴(納品日/支払額/納品個数/その他+備考)を注記。
--   field_schema は現行を引き継ぐ(検収書フォームは React スキーマ駆動で不変)。
--   additive・冪等(再実行すると version が増えるが害はない)。
...
<body>
  ...
  <div class="doc-header">
    <div>
      ...
      {{!-- 分納バッジは廃止: 複数業務明細×分納で「第N回/全M回」の回数集計が不整合に
           なるため削除。分納の内訳は「今回の納品内容」の 今回数量/歩留率 列で表現する。 --}}
      {{#if (gt REVISION 0)}}
  ...
  --}}
  {{#if useGroupedInspection}}
  {{!-- 新フォーマット: 支払日ごとにグループ化。歩留率/数量は出さず、金額(税抜)のみ。
        消費税は支払日(課税仕入れの時期)ごとに端数処理し、書類全体の消費税合計は出さない。 --}}
  <div class="section-title">■ 検収明細（支払日ごと）</div>
  {{#each paymentGroups}}
  <table style="margin-bottom:10px;">
    <thead>
      <tr>
        <th colspan="4" style="text-align:left;background:#e8eef6;">
          {{#if this.isPaid}}支払日{{else}}支払予定日{{/if}}： {{formatDate this.date}}
          {{#if this.isPaid}}<span style="display:inline-block;margin-left:10px;padding:1px 8px;border:1px solid #9ccea9;background:#e7f5ea;color:#237a3a;font-size:8.5pt;font-weight:bold;border-radius:3px;vertical-align:middle;">支払済</span>{{else}}<span style="display:inline-block;margin-left:10px;padding:1px 8px;border:1px dashed #c98a2b;background:#fdf3e0;color:#92400e;font-size:8.5pt;font-weight:bold;border-radius:3px;vertical-align:middle;">支払予定</span>{{/if}}
        </th>
      </tr>
      <tr>
        <th style="width:6%">No.</th>
        <th style="width:56%">成果物・業務内容</th>
        <th style="width:16%">納品日</th>
        <th style="width:22%">支払対価（税抜）</th>
      </tr>
    </thead>
    <tbody>
      {{#each this.lines}}
      <tr>
        ...
        <td class="right">¥{{formatYen amount_ex_tax}}</td>
      </tr>
      {{#if hasChange}}
      {{!-- 発注条件からの変更履歴(明細別)。変更項目＋備考(理由)を明細直下に注記。 --}}
      <tr>
        <td></td>
        <td colspan="3" style="background:#fff7ed;border-top:none;font-size:8.5pt;color:#9a3412;">
          ⚠ 発注条件からの変更{{#if changeLabel}}：{{changeLabel}}{{/if}}{{#if changeNote}}　／　備考：{{changeNote}}{{/if}}
        </td>
      </tr>
      {{/if}}
      {{/each}}
      <tr class="subtotal-row"><td colspan="3" class="right">小計（税抜）</td><td class="right">¥{{this.subtotalStr}}</td></tr>
      <tr class="subtotal-row"><td colspan="3" class="right">消費税（{{this.taxRate}}%）</td><td class="right">¥{{this.taxAmountStr}}</td></tr>
      <tr class="total-row"><td colspan="3" class="right">源泉徴収税額計算前　{{#if this.isPaid}}支払額{{else}}支払予定額{{/if}}（税込）</td><td class="right">¥{{this.totalIncTaxStr}}</td></tr>
    </tbody>
  </table>
  {{/each}}
  <div style="font-size:8.5pt;color:#555;margin-top:2px;">※ 消費税は支払日（課税仕入れの時期）ごとに税率別で端数処理して各グループに表示しています。<span style="color:#237a3a;font-weight:bold;">支払済</span>＝既にお支払い済み／<span style="color:#92400e;font-weight:bold;">支払予定</span>＝記載の予定日にお支払い。</div>
  {{else}}
  ...
  <table style="margin-bottom:4px;">
    <thead>
      <tr>
        ...
        <th style="width:38%">成果物・業務内容</th>
        <th style="width:10%">今回数量</th>
        <th style="width:9%">歩留率</th>
        <th style="width:12%">納品日</th>
        <th style="width:12%">支払日</th>
        <th style="width:14%">支払対価（税抜）</th>
      </tr>
    </thead>
    <tbody>
      ...
      {{#if (gt (length delivery_line_items) 0)}}
        {{#each delivery_line_items}}
          <tr>
            ...
            {{!-- 明細別の実支払日。過去に別々の日付で支払済みの検収を1枚に集約する用途。
                 未入力なら文書全体の支払期日にフォールバック。 --}}
            <td class="center">{{#if paid_date}}{{formatDate paid_date}}{{else}}{{#if ../paymentDueDate}}{{formatDate ../paymentDueDate}}{{else}}—{{/if}}{{/if}}</td>
      ...
      {{else}}
        <tr>
          ...
          <td class="center">{{#if paymentDueDate}}{{formatDate paymentDueDate}}{{else}}—{{/if}}</td>
      ...
      {{!-- Phase 23.6.16: その他手数料がある場合、消費税は末尾の「支払額サマリー」で
           ...
           ここでは検収の税抜小計のみ表示)。 --}}
      <tr class="total-row">
        <td colspan="6" class="right">検収 小計（税抜）</td>
        <td class="right">¥{{deliveredAmountStr}}</td>
      </tr>
      {{else}}
      <tr>
      ...
      <tr class="total-row">
        <td colspan="6" class="right">源泉徴収税計算前　検収金額(税込)</td>
  ...
  {{!-- Phase 23.6.16: 支払額サマリー（検収書は仕入明細書として消費税法57条の4 の
       ...
       新フォーマット(支払日グループ)では書類全体の税サマリーは出さない。 --}}
  {{#unless useGroupedInspection}}
  {{#if otherFeesTaxable}}
  ...
  {{/if}}
  {{/unless}}{{!-- /useGroupedInspection: 書類全体の税サマリー・総支払額は新フォーマットでは非表示 --}}
...
$html_inspection_certificate$,
    ...
    '検収書: 支払日グループ化フォーマット+明細別変更履歴 (PR #478)',
    'migration-0152'
  FROM t
