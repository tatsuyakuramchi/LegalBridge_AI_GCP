-- 0056_po_zero_fee_display.sql
-- 発注書: 受注者帰属で業務報酬が0のとき、単価/数量/金額を「-」表示にし(無報酬と誤認させない)、
--   注記を「業務委託報酬は利用許諾料に含まれる」に変更。業務明細側の計算方式は業務報酬(FIXED)を表示し
--   料率(利用許諾)の表示は抑制(料率は利用許諾条件セクションに表示)。
...
<body>
  ...
  <div style="margin-top:4px; font-size:8.5pt; color:#555;">※ 上記は成果物のIPが受注者に帰属するための利用許諾料条件です。発注の確定額（業務委託小計）には含まれず、別途、利用許諾料計算書により算定・支払われます。</div>
  {{/if}}
