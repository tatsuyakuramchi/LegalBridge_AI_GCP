-- 0124_po_payment_method_label_ja.sql
-- 発注書(purchase_order)テンプレの「支払方法」ラベルを英語 enum から日本語へ:
--   FIXED→固定額 / SUBSCRIPTION→定期払い /
--   ROYALTY→ 受注者帰属=利用許諾料・発注者帰属=インセンティブ報酬。
--   本番 worker/search-api は TEMPLATE_SOURCE=db のため DB 版 current を新版へ貼替。
...
BEGIN
  ...
  IF tid IS NULL THEN
    RAISE NOTICE '0124: purchase_order template not found, skipping';
    RETURN;
  END IF;
...
$po_html$ THEN
    RAISE NOTICE '0124: purchase_order already up to date, skipping';
    RETURN;
  END IF;
...
$po_html$,
          cur_schema,
          '0124: 支払方法ラベル日本語化(固定額/定期払い/利用許諾料・インセンティブ報酬)',
          'migration-0124')
