  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print {
      ...
      /* 署名欄は割らない。頭書き表は「行単位」で割れないようにし、表全体 avoid による
         大きな空白/溢れを防ぐ(長い特記事項でも溢れず改ページできる)。 */
      .head-signature { page-break-inside: avoid; break-inside: avoid; }
      .tobogaki tr { page-break-inside: avoid; break-inside: avoid; }
      /* 孤立行(1行だけ次/前ページに残る)防止を本文条項にも効かせる(p 限定をやめる)。 */
      p, .preamble, .clause, .sub-clause { orphans: 2; widows: 2; }
      .article-title { page-break-after: avoid; break-after: avoid; }
      /* 割りたくない塊に付与するユーティリティ。 */
  ...
  <table class="tobogaki">
    <tbody>
      ...
      <tr>
        ...
        <td class="col-value">
          {{VENDOR_ADDRESS}}<br>
          {{VENDOR_NAME}}{{#if (or VENDOR_IS_CORPORATION VENDOR_REP)}}<br>
          {{VENDOR_REP}}{{/if}}
        </td>
      </tr>
      <tr>
    ...
    <div class="sig-grid">
      <div class="sig-party">
        ...
        <div class="sig-name-row">
          {{#if (or VENDOR_IS_CORPORATION VENDOR_REP)}}<span>{{VENDOR_REP}}</span>{{else}}<span></span>{{/if}}
