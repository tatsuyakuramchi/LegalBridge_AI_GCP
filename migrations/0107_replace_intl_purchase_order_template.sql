-- 0107_replace_intl_purchase_order_template.sql
-- 海外発注書(intl_purchase_order)テンプレを国内発注書の新デザイン(0088)と同一構造へ差し替え＋外貨対応。
--   金額は formatYen でなく CURRENCY コード + formatMoney (小数2桁対応・要 worker/api 新ヘルパー)、
--   SUBSCRIPTION サイクルは cycleLabelEn で英語正規化。Schedule A (Part 2) は不変更。
--   disk: services/worker/templates/intl_purchase_order.html と同一内容。
--   TEMPLATE_SOURCE=db の worker / search-api(プレビュー) はこの DB 版を読むため新版を current に。
--   field_schema は現行版から引き継ぐ（フォーム項目は不変）。
--   ※ 適用前に formatMoney / cycleLabelEn を持つ worker (release/worker ea85493) /
--     search-api (release/api 5ed1a5e) がデプロイされていること (未登録ヘルパーはレンダリングエラー)。
DO $mig$
DECLARE
  tid INTEGER;
  cur_html TEXT;
...
BEGIN
  ...
  IF tid IS NULL THEN
    RAISE NOTICE '0107: intl_purchase_order template not found, skipping';
    RETURN;
  END IF;

  IF cur_html IS NOT DISTINCT FROM $intl_html$<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
  ...
  <tbody>
    ...
    <tr class="item-detail">
      ...
      <td colspan="4">
        ...
        　／　Delivery: {{#if (eq calc_method "SUBSCRIPTION")}}{{cycleLabelEn cycle interval_unit interval_count}}{{#if (eq billing_day 0)}} / last day of each period{{else}}{{#if billing_day}}{{#if (gt billing_day 30)}} / last day of each period{{else}} / day {{billing_day}} of each period{{/if}}{{/if}}{{/if}}{{else}}{{#if delivery_date}}{{formatDateCompact delivery_date}}{{else}}—{{/if}}{{/if}}
        　／　Payment Date: {{#if (eq calc_method "SUBSCRIPTION")}}{{#if term_start}}{{formatDateCompact term_start}}{{else}}—{{/if}} – {{#if term_end}}{{formatDateCompact term_end}}{{else}}ongoing{{/if}}{{else}}{{#if (eq calc_method "ROYALTY")}}{{#unless (gt (or amount_ex_tax amount) 0)}}Per Royalty Statement{{else}}{{#if payment_date}}{{formatDateCompact payment_date}}{{else}}—{{/if}}{{/unless}}{{else}}{{#if payment_date}}{{formatDateCompact payment_date}}{{else}}—{{/if}}{{/if}}{{/if}}
...
$intl_html$ THEN
    RAISE NOTICE '0107: intl_purchase_order html already current, skipping';
    RETURN;
  END IF;
...
$intl_html$,
          ...
          '海外発注書テンプレ差し替え（新デザイン・外貨対応）(0107)', 'migration-0107')
