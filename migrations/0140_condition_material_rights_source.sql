...
--   condition_lines.material_rights_source_id を追加し、条件が指す素材
--   (condition_lines.source_material_id)の主要権利根源(material_rights_sources.is_primary)を
--   1:1 でバックフィルする。これで利用許諾計算書の利用料名目を権利根源から解決する土台になる。
--   ※ material_ref_id は documents 側の列で condition_lines には無いため、source_material_id を使う。
...
-- バックフィル: 条件が指す素材(source_material_id)の主要権利根源(is_primary)を紐付ける。
--   既に付いている行は触らない(冪等)。
...
UPDATE condition_lines cl
 ...
 WHERE cl.material_rights_source_id IS NULL
   ...
   AND mrs.is_primary
   AND mrs.material_id = cl.source_material_id;
