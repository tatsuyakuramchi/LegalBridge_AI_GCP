-- 0088_replace_purchase_order_template_new_design.sql
-- 発注書(purchase_order)テンプレを新デザイン（業務明細2行レイアウト）へ差し替え。
--   データ契約は不変（items/financial_conditions/other_fees/expenses/各合計・受領確認等）。
--   利用許諾条件テーブル・サブスク/支払スケジュール・単一明細フォールバックは温存。
...
BEGIN
  ...
  IF tid IS NULL THEN
    RAISE NOTICE '0088: purchase_order template not found, skipping';
    RETURN;
  END IF;
  ...
  <tbody>
    ...
    <tr class="item-detail">
      ...
      <td colspan="4">
        ...
        　／　納期：{{#if (eq calc_method "SUBSCRIPTION")}}{{billingDayLabel billing_day cycle}}{{else}}{{formatDate delivery_date}}{{/if}}
        　／　支払日：{{#if (eq calc_method "SUBSCRIPTION")}}{{#if term_start}}{{formatDateCompact term_start}}{{else}}—{{/if}} 〜 {{#if term_end}}{{formatDateCompact term_end}}{{else}}継続中{{/if}}{{else}}{{#if (eq calc_method "ROYALTY")}}{{#unless (gt (or amount_ex_tax amount) 0)}}利用許諾料計算書の通り{{else}}{{formatDate payment_date}}{{/unless}}{{else}}{{formatDate payment_date}}{{/if}}{{/if}}
...
$po_html$ THEN
    RAISE NOTICE '0088: purchase_order html already current, skipping';
    RETURN;
  END IF;
...
$po_html$,
          ...
          '発注書テンプレ差し替え（新デザイン・2行明細）(0088)', 'migration-0088')
