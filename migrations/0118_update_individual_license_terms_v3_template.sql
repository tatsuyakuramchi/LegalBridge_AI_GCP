-- 0118_update_individual_license_terms_v3_template.sql
-- individual_license_terms_v3 テンプレを最新(disk)へ更新して DB(current版)へ反映する。
--   経緯: 0087 で DB 登録済み。ただし sync-templates-to-db.mjs は templates_config.json の
--   キーのみ同期対象で、v3 は config 非キーのため同期されない。したがって v3 のテンプレ変更は
--   本マイグレーション(または Cloud SQL 手動実行)で新版を積む必要がある(0087 と同方式・冪等)。
--   内容: services/worker/templates/individual_license_terms_v3.html と同一。
--   変更点: 1-3 案3再設計 / 1-2 文言(用語・定義・参照) / 署名(署名または印章) /
--           2-3(B)支払日 法人個人分岐 / 3-2 支払日まで報告 / 構成要素の許諾地域・言語(1-3A)。
DO $mig$
DECLARE
  tid INTEGER;
  vid INTEGER;
...
$ilt_v3$,
            '[]'::jsonb,
            'v3 更新: 1-3案3再設計/1-2文言/署名/支払日分岐/構成要素の許諾地域言語 (0118)', 'migration-0118')
