-- 0054_po_license_inline.sql
-- 発注書テンプレ: 受注者帰属(利用許諾料)の明細も業務明細に表示し、金額欄は
--   「委託報酬は利用許諾料に含む（利用許諾条件参照）」として下段の利用許諾条件へ誘導する。
-- document_templates(db モード)を新版へ更新。
  ...
  <table class="items">
    ...
    <tbody>
      ...
      <tr class="{{#if (or spec detailText)}}has-detail{{/if}}">
        ...
        <td colspan="6" style="font-size:8.5pt; color:#92400e; vertical-align:middle;">
          <span class="pay-method-badge" style="background:#fde68a; color:#92400e;">利用許諾</span>
          委託報酬は利用許諾料に含む（下段「利用許諾条件」参照{{#if condition_name}}：{{condition_name}}{{/if}}）
        </td>
        {{else}}
        <td class="center">
