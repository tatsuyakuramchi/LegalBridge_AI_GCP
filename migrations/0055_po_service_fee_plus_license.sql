-- 0055_po_service_fee_plus_license.sql
-- 発注書: 受注者帰属でも「業務報酬(執筆料等)」を確定額として業務明細に表示し、
--   併せて「＋利用許諾条件あり」を示し下段の利用許諾条件へ誘導する。
--   業務報酬が無い場合は金額欄に「利用許諾料のみ」と表示。
  ...
  <table class="items">
    ...
    <tbody>
      ...
      <tr class="{{#if (or spec detailText)}}has-detail{{/if}}">
        ...
        <td class="right">{{#if (eq deliverable_ownership "受注者")}}{{#if (gt (or amount_ex_tax amount) 0)}}{{formatYen (or amount_ex_tax amount)}}{{else}}<span style="font-size:7.5pt;color:#92400e;">利用許諾料のみ</span>{{/if}}{{else}}{{formatYen (or amount_ex_tax amount)}}{{/if}}</td>
      </tr>
