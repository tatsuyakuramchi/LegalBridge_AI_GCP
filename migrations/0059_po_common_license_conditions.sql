-- 0059_po_common_license_conditions.sql
-- 発注書: 受注者帰属の成果物に適用する「利用許諾条件（共通）」を導入。
--   - capability_financial_conditions に applies_scope(適用範囲/対象成果物) 列を追加。
--   - 発注書フォームの financial_conditions(共通条件) を worker が保存し、テンプレは
--     利用許諾条件セクションを「共通条件＋適用範囲」で描画(共通条件が無い旧発注書は
--     従来の per-line 明細フォールバックで描画)。
-- disk テンプレ(services/worker/templates/purchase_order.html)と整合。field_schema は
-- フラット項目に変更が無いため 0056 と同一(financial_conditions は動的配列でカスタムフォーム管理)。
...
<body>
  ...
  <table class="items">
    ...
    <tbody>
      {{#if items}}
      {{#each items}}
      {{!-- 受注者帰属(利用許諾料)の行も業務明細に出すが、金額は出さず「利用許諾料に含む」とし
           下段の「利用許諾条件」へ誘導する。 --}}
      ...
      <tr class="{{#if (or spec detailText)}}has-detail{{/if}}">
        ...
        <td>
          ...
          {{#if (eq deliverable_ownership "受注者")}}<div style="font-size:7.5pt; color:#92400e; margin-top:2px; font-weight:700;">＋利用許諾条件あり（下段「利用許諾条件」参照{{#if condition_name}}：{{condition_name}}{{/if}}）</div>{{/if}}
        </td>
        <td class="center">
          {{!-- Phase 13: 計算方式バッジ + 支払条件
               Phase 22.8: SUBSCRIPTION は周期ラベル "月次" 等を補足表示 --}}
          <span class="pay-method-badge">{{#if (eq deliverable_ownership "受注者")}}FIXED{{else}}{{or calc_method payment_method_display payment_method "FIXED"}}{{/if}}</span>
          {{#if (eq calc_method "ROYALTY")}}{{#unless (eq deliverable_ownership "受注者")}}
        ...
        <td class="right">{{#if (eq deliverable_ownership "受注者")}}{{#if (gt (or amount_ex_tax amount) 0)}}{{or quantity qty}}{{else}}<span style="color:#888;">-</span>{{/if}}{{else}}{{or quantity qty}}{{/if}}</td>
        <td class="right">{{#if (eq deliverable_ownership "受注者")}}{{#if (gt (or amount_ex_tax amount) 0)}}{{formatYen (or unit_price unitPrice)}}{{else}}<span style="color:#888;">-</span>{{/if}}{{else}}{{formatYen (or unit_price unitPrice)}}{{/if}}</td>
        <td class="right">{{#if (eq deliverable_ownership "受注者")}}{{#if (gt (or amount_ex_tax amount) 0)}}{{formatYen (or amount_ex_tax amount)}}{{else}}<span style="color:#888;">-</span><div style="font-size:7pt;color:#92400e;">業務委託報酬は利用許諾料に含まれる</div>{{/if}}{{else}}{{formatYen (or amount_ex_tax amount)}}{{/if}}</td>
      </tr>
  ...
  {{!-- 受注者帰属(利用許諾料)の条件。成果物のIPが受注者に帰属するため利用許諾料として扱い、
       発注の確定額には含めない。利用許諾料計算書・分配に連動(worker 保存時に金銭条件へ振り分け)。 --}}
  {{#if has_license_conditions}}
  <div class="section-mark">■ 利用許諾条件（受注者帰属・確定額外）</div>
  <table class="items">
    <thead>
      <tr>
        <th style="width:4%;">#</th>
        <th style="width:26%;">条件名称 / 区分</th>
        <th style="width:34%;">計算式</th>
        <th style="width:18%;">料率 / 基準</th>
        <th style="width:18%;">MG / AG</th>
      </tr>
    </thead>
    <tbody>
      {{#if financial_conditions.length}}
      {{!-- 共通利用許諾条件: 受注者帰属の成果物に一括適用。適用範囲を併記。 --}}
      {{#each financial_conditions}}
      <tr>
        <td class="center">{{or condition_no (index1 @index)}}</td>
        <td>{{#if condition_name}}{{condition_name}}{{else}}利用許諾条件{{/if}}{{#if region_language_label}}<div style="font-size:7.5pt;color:#666;">{{region_language_label}}</div>{{/if}}</td>
        <td>{{#if (eq calc_type "BASE_QTY_RATE")}}基準価格 × 個数 × 料率{{/if}}{{#if (eq calc_type "BASE_RATE")}}基準価格 × 料率{{/if}}{{#if (eq calc_type "FIXED")}}固定値（{{#if (eq fixed_kind "INSTALLMENT")}}分割{{else}}一括{{/if}}）{{/if}}{{#if (eq calc_type "SUBSCRIPTION")}}サブスク（{{#if (eq subscription_cycle "ANNUAL")}}年払い{{else}}月払い{{/if}}）{{/if}}{{#if formula_text}}<div style="font-size:7.5pt;color:#666;">{{formula_text}}</div>{{/if}}</td>
        <td class="right">{{#if rate_pct}}{{rate_pct}}%{{/if}}{{#if base_price_label}}<div style="font-size:7.5pt;color:#666;">{{base_price_label}}</div>{{/if}}</td>
        <td class="right">{{#if (eq guarantee_type "MG")}}MG {{formatYen mg_amount}}{{/if}}{{#if (eq guarantee_type "AG")}}AG {{formatYen ag_amount}}{{/if}}</td>
      </tr>
      {{#if applies_scope}}
      <tr><td style="border-top:0;"></td><td colspan="4" style="border-top:0; font-size:7.5pt; color:#444;"><strong>適用範囲:</strong> {{applies_scope}}</td></tr>
      {{/if}}
      {{/each}}
      {{else}}
      {{!-- 後方互換: 旧 per-line 条件(受注者帰属明細ごと)。共通条件が無い既存発注書向け。 --}}
      {{#each items}}
      {{#if (eq deliverable_ownership "受注者")}}
      <tr>
        <td class="center">{{or line_no (index1 @index)}}</td>
        <td>{{#if condition_name}}{{condition_name}}{{else}}{{item_name}}{{/if}}{{#if region_language_label}}<div style="font-size:7.5pt;color:#666;">{{region_language_label}}</div>{{/if}}</td>
        <td>{{#if (eq calc_type "BASE_QTY_RATE")}}基準価格 × 個数 × 料率{{/if}}{{#if (eq calc_type "BASE_RATE")}}基準価格 × 料率{{/if}}{{#if (eq calc_type "FIXED")}}固定値（{{#if (eq fixed_kind "INSTALLMENT")}}分割{{else}}一括{{/if}}）{{/if}}{{#if (eq calc_type "SUBSCRIPTION")}}サブスク（{{#if (eq subscription_cycle "ANNUAL")}}年払い{{else}}月払い{{/if}}）{{/if}}{{#if formula_text}}<div style="font-size:7.5pt;color:#666;">{{formula_text}}</div>{{/if}}</td>
        <td class="right">{{#if rate_pct}}{{rate_pct}}%{{/if}}{{#if base_price_label}}<div style="font-size:7.5pt;color:#666;">{{base_price_label}}</div>{{/if}}</td>
        <td class="right">{{#if (eq guarantee_type "MG")}}MG {{formatYen mg_amount}}{{/if}}{{#if (eq guarantee_type "AG")}}AG {{formatYen ag_amount}}{{/if}}</td>
      </tr>
  ...
  <div style="margin-top:4px; font-size:8.5pt; color:#555;">※ 上記は成果物のIPが受注者に帰属するための利用許諾料条件です。受注者帰属の各成果物に適用され、発注の確定額（業務委託小計）には含まれず、別途、利用許諾料計算書により算定・支払われます。</div>
  {{/if}}
