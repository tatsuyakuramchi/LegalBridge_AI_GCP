-- 0087_register_individual_license_terms_v3_template.sql
-- 個別利用許諾 v3（マトリクス：取引形態×構成要素LC）テンプレートを document_templates へ登録。
--   worker は TEMPLATE_SOURCE=db のとき DB の current 版を読む（未ヒット時のみ disk フォールバック）。
--   search-api のひな型プレビューは DB のみ（フォールバック無し）のため、DB 登録が必須。
--   disk: services/worker/templates/individual_license_terms_v3.html と同一内容。
--   既存 individual_license_terms（旧フラット）はそのまま。v3 は v3_conds 検出時に worker が選択。
DO $mig$
DECLARE
  tid INTEGER;
  vid INTEGER;
...
<table class="avoid-break">
  ...
  <tr><td class="k">許諾地域</td><td>{{maxRegion}}。本契約に基づく各金銭条件の許諾地域は、本項に定める範囲内で 2-1 金銭条件マスタに定める。</td></tr>
  <tr><td class="k">許諾言語</td><td>{{maxLanguage}}。本契約に基づく各金銭条件の許諾言語は、本項に定める範囲内で 2-1 金銭条件マスタに定める。</td></tr>
...
<p class="lead">本契約に基づくロイヤリティは、各金銭条件において定める基準価格、料率、個数および固定額により、次の算定式に従い計算する。</p>
<table class="formula-box"><tr><td>ロイヤリティ ＝ 基準価格 × 料率 × 個数 ± 固定額</td></tr></table>
<p class="lead">各パラメータは次のとおり定める。</p>
<table class="avoid-break" style="font-size:8.4pt;">
  <tr><td class="k" style="width:92pt;">基準価格</td><td>製造者および販売者の組み合わせにより決まる価格であり、1-3(A) 基準価格表に定める。</td></tr>
  <tr><td class="k">料率</td><td><b>【加算型】</b> の条件にあっては 1-3 に定める各構成要素の料率を合算した値とし、<b>【非加算型】</b> の条件にあっては 1-3 を参照のうえ 2-1 に明記した実効料率とする。</td></tr>
  <tr><td class="k">個数</td><td>製造・販売の数量とする。ただし、個数の概念を適用しない条件にあっては、これを 1 とする。</td></tr>
  <tr><td class="k">AG（前払保証）</td><td>契約締結時に Licensor へ支払う前払保証額であり、2-1 に定める。以後のロイヤリティと順次相殺する。AG を適用しない条件にあっては、これを 0 とする。</td></tr>
  <tr><td class="k">MG（最低保証）</td><td>計算期間末時点で累積ロイヤリティが MG に未達の場合、Licensee が差額を追加支払う最低保証額であり、2-1 に定める。MG を適用しない条件にあっては、これを 0 とする。</td></tr>
</table>
<p class="leg">※ 計算例は 2-1 金銭条件マスタの脚注に記載する。</p>

<!-- 1-3 差込 -->
<h3 class="sec">1-3　許諾対象（構成要素台帳）</h3>
<p class="lead">本契約が許諾する対象作品は次表の構成要素（LC）の総体をいう。取引形態の個数・固定額・通貨は 2-1 金銭条件マスタによる。</p>

<p class="sub-title">(A)　基準価格表</p>
<table class="avoid-break" style="font-size:8.4pt;">
  <tr>
    <th style="width:34pt;">条件</th>
    <th style="width:80pt;">製造者</th>
    <th style="width:80pt;">販売者</th>
    <th style="width:62pt;" class="c">地域<br><span style="font-size:6.8pt;font-weight:400;">（最大）</span></th>
    <th style="width:62pt;" class="c">言語<br><span style="font-size:6.8pt;font-weight:400;">（最大）</span></th>
    <th>基準価格</th>
  </tr>
  {{#each conds}}
  <tr>
    <td class="c">{{condLabel}}</td>
    <td>{{manufacturer}}</td>
    <td>{{seller}}</td>
    <td class="c">{{maxCondRegion}}</td>
    <td class="c">{{maxCondLang}}</td>
    <td>{{basePrice}}</td>
  </tr>
  {{/each}}
</table>
<p class="leg">製造者・販売者の組み合わせが基準価格を決定する。地域・言語は当該取引形態において許諾できる最大スコープ（過去の合意により決まっているフレーム）を示す。2-1 における地域・言語は、本表の範囲内で今回の製品に適用する実際のスコープを定める。取引形態が増える場合は行（条件4 以降）を追加する。</p>

<p class="sub-title">(B)　料率表（構成要素 × 取引条件）</p>
<table class="avoid-break">
  <tr>
    <th rowspan="2" style="width:44pt;">区分</th>
    <th rowspan="2">構成要素</th>
    <th rowspan="2" style="width:96pt;">権利元</th>
    <th colspan="{{condCount}}" class="c" style="background:#ececec;">取引条件別料率</th>
  </tr>
  <tr>
    {{#each conds}}
    <th style="width:54pt;" class="c">{{condLabel}}<br><span style="font-size:7pt;font-weight:400;">{{condType}}</span></th>
    {{/each}}
  </tr>
  {{#each lcs}}
  <tr>
  ...
  {{else}}
  <tr><td colspan="{{add condCount 3}}" class="c" style="color:#aaa;">構成要素を追加してください</td></tr>
  {{/each}}
</table>
<p class="leg"><b>【加算型】</b> 1-2 で定める料率の算定方法。対象製品に含まれる LC の料率を合算して 2-1 の適用料率を算出する。<b>【非加算型】</b> 各 LC 料率を参照のうえ Licensor・Licensee 間で改めて定めた実効料率を 2-1 に明記する。構成要素が増える場合は行を追加する。取引形態が増える場合は列（条件4 以降）を追加する。権利元が異なる行が生じた場合は特約 4-2(2)（按分）の対象。</p>
...
<p class="lead">各取引形態の4パラメータと通貨を定める。ロイヤリティの計算式は 1-2 による。取引形態が増える場合は本表に行（条件4 以降）を追加する。</p>
<table class="avoid-break" style="font-size:8.4pt;">
  <tr>
    <th style="width:34pt;">条件</th>
    <th style="width:46pt;" class="c">種別</th>
    <th style="width:60pt;" class="c">地域<br><span style="font-size:6.8pt;font-weight:400;">1-1の範囲内</span></th>
    <th style="width:60pt;" class="c">言語<br><span style="font-size:6.8pt;font-weight:400;">1-1の範囲内</span></th>
    <th style="width:96pt;" class="c">適用料率<br><span style="font-size:6.8pt;font-weight:400;">加算型＝1-3(B)の合算</span></th>
    <th style="width:36pt;" class="c">個数<br><span style="font-size:6.8pt;font-weight:400;">なし＝1</span></th>
    <th style="width:52pt;" class="c">AG（前払保証）<br><span style="font-size:6.8pt;font-weight:400;">なし＝0</span></th>
    <th style="width:52pt;" class="c">MG（最低保証）<br><span style="font-size:6.8pt;font-weight:400;">なし＝0</span></th>
    <th style="width:32pt;" class="c">通貨</th>
  </tr>
  {{#each conds}}
  <tr>
    <td class="c">{{condLabel}}</td>
    <td class="c"><b>{{condType}}</b></td>
...
<p class="leg">基準価格・取引構造は 1-3(A) 基準価格表による。地域・言語は 1-1 に定める許諾地域・許諾言語の範囲内、かつ 1-3(A) の最大スコープ以内で各条件ごとに定める。共通条件：算定期間＝製造時。料率・固定額は選択通貨建て。</p>
...
<table class="avoid-break">
  ...
  <tr><td>3-2　報告</td><td>製造時（製造都度）</td><td>製造（印刷）ベースで初回・再生産ごとに報告する。報告内容＝言語別製造数、希望小売価格（MSRP）、ロイヤリティ計算明細（販促・サンプル等の除外数を含む）。</td></tr>
...
<div class="sign">
  <div class="sigcol">
    ...
    <div class="sigrow"><div class="rk">会社名</div><div class="rv">{{licensorName}}　〔電子印影〕</div></div>
  ...
  <div class="sigcol">
    ...
    <div class="sigrow"><div class="rk">会社名</div><div class="rv">{{licenseeName}}　〔電子印影〕</div></div>
