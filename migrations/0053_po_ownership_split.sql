-- 0053_po_ownership_split.sql
-- 発注書テンプレ(purchase_order)を成果物帰属の振り分け表示版へ更新:
--   業務委託明細(発注者帰属)と利用許諾条件(受注者帰属・確定額外)を2セクションで描画。
-- rate_pct 列は 0052 で追加済み。document_templates(db モード)を新版へ更新。
...
<body>
  ...
  <table class="items">
    ...
    <tbody>
      {{#if items}}
      {{#each items}}
      {{!-- 受注者帰属(利用許諾料)の行は業務委託明細に出さず、後段の「利用許諾条件」へ。 --}}
      {{#unless (eq deliverable_ownership "受注者")}}
