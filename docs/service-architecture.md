...
| **D3 コード重複** | **意図的分岐**(共有lib化しない)。両者の契約面は「DBスキーマ + 互換ビュー」 | Search=新スキーマで育成 / worker=互換ビューで凍結、と設計上分岐する。凍結側に共有libを投資しても無駄 |
...
1. **採番(`document_sequences`)**: 生成書類は worker 採番のまま。**Search 登録の契約**は ①外部番号(LegalOn等)をそのまま使う / ②Search 専用採番列を新設、のどちらにするか。
2. **アラート送信**: `alerts` レコードは Search が書くが、**Slack 送信ジョブ**は当面 worker(既存)。新通知器を Search に作るのは後続フェーズでよいか。
3. **`contracts.lifecycle_stage` 列**: 締結進捗は Search 所有でよいか(Backlog webhook 由来のステータスは worker → `issue_workflows` に残し、Search 側 `lifecycle_stage` へマッピング同期)。
