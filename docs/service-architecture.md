...
### 8.7 実装(確定): worker デプロイ・パイプラインに統合
当初の「独立 `legalbridge-migrate` トリガー」と「worker 起動時 `initDb()`」の 2 系統運用は、設定ズレ(`_INSTANCE_CONNECTION_NAME` 未指定 → Cloud SQL バインドが外れ socket `ENOENT`)や二重起動で不安定だった。これを**1 系統に統合**した。

- **`cloudbuild-worker.yaml` ① migration ステップ**が `migrations/` ランナー(Cloud Run Job `legalbridge-migrate`)を **build → deploy(必ず `--set-cloudsql-instances` を付与)→ execute --wait** する。成功してから ② worker をデプロイ(失敗時は worker 未更新=旧 revision 継続)。
- 接続は `lb_migrate`(Secret `lb-migrate-database-url`)。アプリロールに DDL を与えない。
- **worker `initDb()` は既定で実行しない**(`RUN_INIT_DB=true` の時だけ後方互換)。`services/worker/server.ts` で gate。
- **独立 `legalbridge-migrate` トリガーは無効化**する(このパイプラインが Job を駆動するため不要)。手動・緊急適用は `cloudbuild-migrate.yaml` を `gcloud builds submit` で単発実行。
- 結果: スキーマ所有は `migrations/` 1 か所、適用契機は `release/worker` push 1 本、起動レース無し、最小権限維持。
