...
| 第2弾 | 書込み経路で contract_id を付与(文書生成/インポート/締結登録)。版ファミリのミラー contracts 重複を財務参照(payments/royalty_statements.contract_id)を付け替えたうえで統合・掃除。dataLinkage の孤児判定を contract_id 基準へ | 未着手 |
