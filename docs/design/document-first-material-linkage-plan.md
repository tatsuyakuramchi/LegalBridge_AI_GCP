...
| 1 | 文書フォーム明細に 作品/原作 セレクタ（なければ作成）＋ 原作マテリアルへの紐づけ（材料ファースト 1材料:N）を追加 | DocumentForm.tsx | ✅ フォーム層実装（作品連動スイッチ／対象作品セレクタ／**材料ファースト**: 既定で全条件を軸マテリアル(素材／原作本体)へ束ね、行で別マテリアルに上書き可。payment_scheme は条件行 calc_type を流用）。**入力順整理**: 縦串(作品→原作→軸マテリアル→金銭条件→紐づけ)が隣接するよう、金銭条件(FinancialConditionTable)を §3 直後「3-2」へ移設し紐づけをその直後に配置（個別利用許諾条件書フォーム） |
