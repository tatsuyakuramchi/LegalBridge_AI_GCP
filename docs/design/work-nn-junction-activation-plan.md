...
| **4. 移行・経路一本化** | 既存フラット(`work_id`+`source_material_id`)→中間表バックフィル(migration 0080・冪等)。`syncWorkComponentLink` の「他コンポーネント除去」を撤廃し ⑧attach-work も加算化(共有を壊さない)。**フラット列の物理廃止はデュアルラン検証後の別移行に先送り**(実データ依存)。 | migration 0080 ＋ workModel.ts | ✅ 実装（`feat/work-nn-junction-stage4`） |
...
- **書込経路の二系統**: ✅ Stage 4 で `syncWorkComponentLink` の「他コンポーネント除去」を撤廃し、⑧ `attach-work` の追加(ADD)も加算化＝ピッカーの共有結線を壊さない。**残**: ⑧の「外す」(work_id=null)は今もその明細の中間表結線を全削除する＝**共有明細は ⑧ ではなくピッカーの作品別「外す」で解除する**運用境界。完全収斂(⑧を加算 unlink へ委譲 or 廃止)は後続。
- **フラット列の物理廃止**: 0080 バックフィル後もフラット列(`work_id`/`source_material_id`)は残す。グラフは中間表∪フラットで読むため安全。**本番でデュアルラン→中間表とフラットの parity 検証→フラット読み撤去→列 DROP** の順で別途実施。
- **検証前提**: 0078/0079/0080 は本番未適用ドラフト。バックフィル件数・コンポーネント採番は実データで要確認。
