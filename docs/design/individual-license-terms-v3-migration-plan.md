...
9. ~~跨ぎ原作（構成要素が複数原作にまたがる）~~ → **確定（C-3 拡張・ユーザー 2026-06）**: 作品Cは**複数原作のマテリアルを束ねられる**（例: A=過去利用許諾の原作X 5% ＋ B=過去成果物の原作Y 2% → C 7%）。**サーバ**: `decomposeAddonConditionToLcCells` がセルの `material_code` から**所属原作を全体検索で解決**（`work_materials`→`work_id`）、各セルを正しい原作へ結線しつつ `work_components` は作品Cへ跨いで組み込む。新規（件名のみ）は文書の原作へ作成。**フォーム**: LC選択は「**文書原作＋追加原作**」方式（既定=文書の原作、「他原作を追加」で全原作の素材を原作グループ付きで選択）。`useAppData().ledgers` を素材源に流用（新API不要）。非加算型は単一原作のまま（本質的に作品一括）。
