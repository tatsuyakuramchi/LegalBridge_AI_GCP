...
## 8. 進捗
- **Phase 1（DDL）✅ 検証済**: `migrations/0089_simplify_condition_core.sql`。
  ローカル postgres にベースライン(0001-0088)＋0089 を適用して構造を確認。
  廃止11テーブル削除／`work_material_uses` 新設／`condition_lines` 新列＋旧列掃除／
  `documents` 統合列／`condition_receipts` 付替え／依存ビュー3つ撤去。
  ⚠ **destructive かつコード未対応のため feature ブランチのみ。release/worker へは Phase 2-4 完了後**。
- **Phase 2（書き込み）🚧 着手**: 核プリミティブ `services/worker/src/lib/conditionWrite.ts`
  (`upsertDocumentConditions`) を新設・test DB で検証(10/10)。CL を直接 upsert し
  material_code→source_material_id/source_work_id へ結線（原作ビューに出る＝Master症状の構造的解消）。
  次: server.ts の Master/文書/発注/v3 保存パスへ配線し cfc/cli upsert を撤廃。
  - `documentSave.ts` の `upsertMasterContract`（documents統合upsert＋条件→CL・本体アンカー）
    を新設・test DBで end-to-end 検証(8/8)。Master登録CLが原作ビューに出る＝症状解消を実証。
    残: server.ts `/api/master/contracts` を本関数の薄いラッパへ置換 → 文書/発注/v3 へ展開。
  - ✅ server.ts `/api/master/contracts` POST/PUT/PATCH を upsertMasterContract へ置換済
    (contract_capabilities INSERT/UPDATE・cfc/cli/expenses/other_fees upsert・別documents INSERT を撤去、
    ringi リンクは温存、PATCH /status は documents へ付替え)。worker typecheck 0 errors。
    line_items/expenses/other_fees もタイプ別 line_no レンジで CL へ。documents は updated_at 等も追加。
    残: 文書作成/発注/v3 保存パス、scopes の documents 付替え。
### 移行手法の確定: **互換ビュー方式（strangler-fig）**
881参照(27ファイル)の全面ハンドリライトを避け、ユーザ確定で互換ビュー方式を採用。
物理スキーマは **documents + condition_lines の2表（真実源）** に単純化しつつ、旧表を同名 VIEW で温存。

- **0089 を互換ビュー版へ刷新（✅検証済）**:
  - `documents` を `contract_capabilities` の**完全スーパーセット化**（不足40列追加＋NOT NULL既定）。
  - `condition_lines` に `document_id`/`group_no`/`legacy_role`/取引形態ヘッダをインライン。
    `capability_id` は廃止せず **document_id ミラー**として温存（旧 `cl.capability_id` 参照の互換）。
  - 旧5表を DROP し**互換 VIEW + INSTEAD OF トリガ**で再作成:
    - `contract_capabilities` → `documents`（auto-updatable＋INSERT upsert トリガ）
    - `capability_financial_conditions`/`_line_items`/`_expenses`/`_other_fees`
      → `condition_lines`（`legacy_role` 振分け／line_no レンジ offset／CHECK整合の payment_scheme 導出／
      source_work_id 自動解決）
  - 周辺レガシー表（ledgers/materials/source_ips/work_components 等）は本移行では**温存**（後段退役）。
- **コード対応（✅）**: 互換ビューは INSTEAD OF トリガ持ちで `INSERT ... ON CONFLICT` 不可のため、
  worker(12) + api(2) の compat INSERT から `ON CONFLICT` 句を除去（トリガが内部 upsert）。
  `conditionWrite.ts` に `capability_id`/`legacy_role` 付与。`syncConditionLinesForCapability` no-op 化。
- **効果**: ~870 の SELECT 参照と ~100 の INSERT/UPDATE/DELETE が**無改修**で新2表へ着地。

- **Phase 3（ビュー再作成）✅**: `condition_line_status_v/balance_v/schedule_v` を新 condition_lines 上に
  旧定義のまま再作成（参照列は全温存・worker+api 37箇所）。代表クエリ(cc×cfc×cli JOIN/status)を EXPLAIN 検証。

検証サマリ（/tmp/pgs, baseline+0089）: 互換 23/23・統合 13/13・worker tsc 0・代表read EXPLAIN OK。
「Master登録CLが原作選択で反映しない」症状＝**構造的に解消**（利用許諾CLへ source_work_id 付与を実証）。

- Phase 4（フロント）— 未着手（現フォームは旧shapeを生成→互換層が受容。新shape直結は後段）
- Phase 5（清掃）— 後段（互換ビュー安定後にヘルパ削除・周辺レガシー表退役・work_material_uses 充填）
