...
### A-1. API: 課題の文書一覧取得
- [ ] `services/api/server.ts` に `GET /api/issues/:issueKey/documents` を追加。
...
### A-2. ルートとページ骨格
- [ ] `src/App.tsx`（ルート定義箇所）に `/issues/:issueKey` を追加。
- [ ] `src/pages/IssueDetailPage.tsx` を新規作成。構成:
...
- [ ] `src/pages/RequestsPage.tsx` の `open()`（L63 付近）を変更: `/documents/new` 直行をやめ、
  `/issues/${key}` へ navigate する。
- [ ] `src/pages/DashboardPage.tsx` の課題行クリックも同様に `/issues/:key` へ。
- 受け入れ基準:
...
### A-3. （任意）文書件数バッジ
- [ ] RequestsPage のカードに `issue.documentCount` を表示（Dashboard と同じデータ源）。
...
- [ ] 採番関数 `issueConditionLineCode()` / `issueWorkCode()` を実装（document_sequences 流用）。

### B-7. Phase B 受け入れ基準
- [ ] worker / api 両サービスが起動し、initDb がエラーなく完走する（新規 DB・既存 DB の両方）。
- [ ] 既存の全機能（文書生成・検収・ロイヤリティ計算・アラート）が無変化で動く。
