...
| P2-3 (読み手移行) | workModel/conditions/contract_works 表示などを「works(kind 両方)」基準に。原作IP一覧 = works WHERE kind='licensed_in'。source_ip_id 参照は legacy_source_ip_id 経由で works に解決。 | なし (次) |
